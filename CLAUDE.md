...
- **Config values are labelled and compared, not dumped.** `api/config_labels.py`
  ...
  **The baseline editor's field list is a STATIC catalogue built from the
  spec.** `api/baselines/field_catalogue.json`, generated by
  `api/scripts/build_field_catalogue.py` from the OpenAPI spec in `spec/` (dev-
  only, gitignored, NOT in the image) and committed. It is keyed exactly like
  the baseline (`<endpoint>.<field-path>`) with each field's type, enum and
  label, so the editor shows every settable field WITHOUT polling a live venue,
  and offers type-aware inputs (bool toggle, enum dropdown, number). Regenerate
  and commit it when RUCKUS reships the spec. It covers the
  `fetch.VENUE_CONFIG_SOURCES` endpoints PLUS the venue-object endpoints
  `shape.config_card` keys on (`apRadioSettings`, `apMeshSettings`, `venue`, …)
  — a test asserts the catalogue is a superset of `ruckus.json`'s keys, so a
